require 'spec_helper'

describe "validates_exclusion_of" do
  let(:attributes) { :admin }
  let(:options) { { :in => [true, false] } }

  let(:model) do
    build_class(:User) do
      include ActiveModel::Validations

      attr_accessor :admin, :manager
    end
  end

  before(:each) do
    model.validates(*attributes, :exclusion => options)
  end

  subject { model.new }

  context "matcher" do
    context "with a single attribute" do
      it { should validate_exclusion_of(:admin, :in => [true, false]) }
      it { should_not validate_exclusion_of(:manager, :in => [true, false]) }
    end

    context "with multiple attributes" do
      let(:attributes) { [:admin, :manager] }

      it { should validate_exclusion_of(:admin, :manager, :in => [true, false]) }
    end

    context "with option :allow_blank" do
      let(:options) { { :in => [true, false], :allow_blank => true } }

      it { should validate_exclusion_of(:admin, :in => [true, false], :allow_blank => true) }
      it { should_not validate_exclusion_of(:admin, :in => [true, false], :allow_blank => false) }
    end

    context "with option :allow_nil" do
      let(:options) { { :in => [true, false], :allow_nil => true } }

      it { should validate_exclusion_of(:admin, :in => [true, false], :allow_nil => true) }
      it { should_not validate_exclusion_of(:admin, :in => [true, false], :allow_nil => false) }
    end

    context "with option :in" do
      let(:options) { { :in => [true, false] } }

      it { should validate_exclusion_of(:admin, :in => [true, false]) }
      it { should_not validate_exclusion_of(:admin, :in => %w(superuser)) }
    end

    context "with option :message" do
      let(:options) { { :in => [true, false], :message => 'is invalid!' } }

      it { should validate_exclusion_of(:admin, :in => [true, false], :message => 'is invalid!') }
      it { should_not validate_exclusion_of(:admin, :in => [true, false], :message => 'invalid') }
    end

    has_unsupported_option(:validate_exclusion_of, :if => :allow_validation)
    has_unsupported_option(:validate_exclusion_of, :unless => :allow_validation)
    has_unknown_option(:validate_exclusion_of, :xxx => true)
  end

  context "description" do
    context "for a defined validation" do
      let(:matcher) { validate_exclusion_of(:admin, :in => [true, false]) }

      it 'contains a description' do
        matcher.description.should ==
          'validate exclusion of admin with {:in=>[true, false]}'
      end
    end

    context "for an undefined validation" do
      let(:matcher) { validate_exclusion_of(:admin, :in => [true, false]) }

      before(:each) { matcher.matches?(subject) }

      it "sets a custom failure message for should" do
        matcher.failure_message.should ==
          'Expected User to validate exclusion of admin with {:in=>[true, false]}'
      end

      it "sets a custom failure message for should not" do
        matcher.negative_failure_message.should ==
          'Did not expect User to validate exclusion of admin with {:in=>[true, false]}'
      end
    end
  end
end
